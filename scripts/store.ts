import { PropertyManager, BaseProperty, Computed, bundle, map } from "hojoki";
import { projector } from '../tags/tags';
import { ScoreManager } from './score_manager';
import { AlbumListManager } from './album_manager';

PropertyManager.defaultImpl.addEventListener(() => {
...
const title = Computed.create(state, albumManager.current.name, scoreManager.viewed)(bundle())
    .c(map(b => {
