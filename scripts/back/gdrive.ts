...
export interface GDriveFile { name: string, id: string }
...
export class WrappedException { constructor(public wrapped: any) {} }
...
class GDriveHandler {
    ...
    async listFiles(q: string, pageSize: number = 120) {
        ...
        do {
            const response = await this.requestHandler.addRequest<gapi.client.drive.FileList>(
    ...
    async getFileItself(fileId: string): Promise<GDriveFile> {
        await this.loadGdrive()
        const resp: Response<Partial<GDriveFile>> = await this.requestHandler.addRequest(gapi.client['drive'].files.get({fileId}))
    ...
    async getContent(id: string, type: 'string' | 'typed-array' = 'string'): Promise<string | Uint8Array> {
        await this.loadGdrive()
        const resp: Response<string> = await this.requestHandler
            .addRequest<string>(gapi.client['drive'].files.get({alt: 'media', fileId: id}))
        const rawstring = resp.body
        return type === 'string' ? rawstring : Uint8Array.from(rawstring.split('').map( e => e.charCodeAt(0)))
    }
}
...
class RequestHandler {
    ...
    async addRequest<A>(r: Request<A>, retryTime: number = 0): Promise<Response<A>> {
        if(retryTime > 10) {
        ...
        const res = await r
        if(res.status === 403) {
            console.log(`403: User Rate Limit Exceeded. wait ${1000 * 2 ** retryTime}ms`)
